...
export default function VisualSectionPerson({
  personId,
  squareUrl,
  rectUrl,
  onUpdated,
  ...
  async function upload(file: File, format: "square" | "rectangle") {
    ...
    try {
      ...
      const res = await api.post("/visuals/person/upload", {
        id_person: personId,
        format,
        base64_image: base64,
      });
      ...
      console.error(e);
      alert("❌ Erreur upload visuel personne");
    }
    setLoading(false);
  }
  ...
  async function resetVisuals() {
    ...
    setLoading(true);
    try {
      const res = await api.post("/visuals/person/reset", {
        id_person: personId,
      });
      ...
      console.error(e);
      alert("❌ Erreur reset visuels personne");
    }
    setLoading(false);
  }
    ...
    <div className="p-4 border rounded bg-white space-y-4">
      ...
      <div className="flex gap-6">
        ...
        <div>
          ...
          {squareUrl ? (
            <img src={squareUrl} className="w-24 h-24 border rounded object-cover" />
          ) : (
          ...
          <input
            ...
            accept="image/*"
            className="mt-2"
            onChange={(e) => e.target.files && upload(e.target.files[0], "square")}
          />
        </div>
        ...
        <div>
          ...
          <input
            ...
            accept="image/*"
            className="mt-2"
            onChange={(e) => e.target.files && upload(e.target.files[0], "rectangle")}
          />
        </div>
      </div>
