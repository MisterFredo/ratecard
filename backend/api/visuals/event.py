...
@router.post("/upload")
def upload_event_visual(payload: EventVisualUpload):
    try:
        # Decode base64
        try:
        ...
        square_filename = f"EVENT_{payload.id_event}_square.jpg"
        rect_filename = f"EVENT_{payload.id_event}_rect.jpg"
        ...
        client.query(
            ...
            job_config=bigquery.QueryJobConfig(
                query_parameters=[
                    ...
                    bigquery.ScalarQueryParameter("id", "STRING", payload.id_event),
                ]
            )
        ).result()
        ...
        raise HTTPException(400, f"Erreur upload visuel événement : {e}")
...
@router.post("/reset")
def reset_event_visual(payload: EventVisualReset):
    try:
        client = get_bigquery_client()
        ...
        raise HTTPException(400, f"Erreur reset visuels événement : {e}")
